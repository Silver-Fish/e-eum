...
def _griffin_lim(S):
    ...
    '''
    # print('griffin---1', flush=True)
    angles = np.exp(2j * np.pi * np.random.rand(*S.shape))
    # print('griffin---2', flush=True)
    S_complex = np.abs(S).astype(np.complex)
    # print('griffin---3', flush=True)
    y = _istft(S_complex * angles)
    # print('griffin---4', flush=True)
    for i in range(hp.griffin_lim_iters):
        # print('for---', flush=True)
        angles = np.exp(1j * np.angle(_stft(y)))
        # print('---for', flush=True)
        y = _istft(S_complex * angles)
    # print('griffin---5', flush=True)
    return y
...
def get_dataset():
    ...
    if hp.dataset == 'LJSpeech':
        ...
        return KSSDatasets(os.path.join(hp.data_path, 'transcript.v.1.4.txt'), os.path.join(hp.data_path, 'kss'))
