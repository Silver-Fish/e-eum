...
const QrCard = () => {
  ...
  useEffect(()=> {
    axios.get(process.env.REACT_APP_API_URL + '/card/qr?typeId=1', config)
    .then((res) =>{
      console.log(res)
      setQrCard(res.data)
    })
      ...
      { (function() {
        if (isAdd === false && isCardStateEdit === false)
          ...
          <>
            ...
            <div className={styles.button_box}>
              <button className={styles.add_button} onClick={addClick}>추가</button>
        ...
        return(
          <>
            ...
            <CardAdd addClick={addClick}></CardAdd>
          </>
        )
